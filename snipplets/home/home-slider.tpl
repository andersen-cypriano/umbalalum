			...
			<div class="row">
				<div class="col section-slider{% if settings.slider_full %} p-0{% endif %}">
					<div class="js-home-slider{% if mobile %}-mobile{% endif %} nube-slider-home swiper-container{% if settings.theme_rounded and not settings.slider_full %} box-rounded{% endif %} swiper-container-horizontal">
						...
						<div class="js-swiper-home-control js-swiper-home-prev{% if mobile %}-mobile{% endif %} swiper-button-prev d-none d-md-block svg-circle-big svg-icon-text{% if settings.icons_solid %} svg-solid{% endif %}">{% include "snipplets/svg/arrow-slider-2.tpl" with {svg_custom_class: "icon-inline icon-2x mr-1"} %}</div>
						<div class="js-swiper-home-control js-swiper-home-next{% if mobile %}-mobile{% endif %} swiper-button-next d-none d-md-block svg-circle-big svg-icon-text{% if settings.icons_solid %} svg-solid{% endif %}">{% include "snipplets/svg/arrow-slider-1.tpl" with {svg_custom_class: "icon-inline icon-2x ml-1"} %}</div>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="col-md mb-4 bee" data-aos-easing="ease" data-aos-delay="1000" data-aos="fade-right">
		{% include "snipplets/svg/bee.tpl" with {svg_custom_class: "bee"} %}
	</div>
{% if not mobile %}
</div>
{% endif %}