...
<section class="account-page">
	<div class="container">
			...
			<div class="col-md-8">
				...
				{% embed "snipplets/forms/form.tpl" with{form_id: 'login-form', submit_custom_class: 'btn-block', submit_text: 'Iniciar sesión' | translate, data_store: 'account-login' } %}
					{% block form_body %}
						...
						{% embed "snipplets/forms/form-input.tpl" with{input_for: 'email', input_placeholder: 'E-mail', type_email: true, input_value: result.email, input_name: 'email', input_custom_class: 'js-account-input', input_required: true } %}
						{% endembed %}
		...
		{% if 'mandatory' not in store.customer_accounts %}
			<p class="mt-3 text-center"> {{ "¿No tenés cuenta aún?" | translate }} {{ "Cadastrar" | translate | a_tag(store.customer_register_url, '', 'btn-link-primary') }}</p>
		{% endif %}
	</div>
</section>