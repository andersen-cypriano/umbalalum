            ...
            <div class="col-md-8">
                {% embed "snipplets/forms/form.tpl" with{form_id: 'info-form', submit_custom_class: 'btn-block', submit_text: 'Guardar cambios' | translate } %}
                    {% block form_body %}
                        ...
                        {% embed "snipplets/forms/form-input.tpl" with{type_tel: true, input_for: 'phone', input_value: result.phone | default(customer.phone), input_name: 'phone', input_id: 'phone', input_placeholder: 'Teléfono (opcional)' | translate } %}
                        {% endembed %}
                    {% endblock %}
                {% endembed %}
